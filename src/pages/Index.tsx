  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {sliderImages.length > 0 && (
        <section className="relative h-[500px] overflow-hidden sm:h-[400px] xs:h-[300px]">
          <div
              ...
              <div key={slide.id} className="w-full h-full flex-shrink-0 relative">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center px-4">
                  <div className="text-center text-white max-w-3xl">
                    ...
                    <p className="text-base sm:text-xl md:text-2xl mb-8">{slide.subtitle}</p>
                    <Link to="/products">
          ...
          <button
            onClick={prevSlide}
            className="absolute left-2 sm:left-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-20 hover:bg-opacity-30 text-white p-2 rounded-full"
          >
            <ChevronLeft className="h-5 sm:h-6 w-5 sm:w-6" />
          </button>
          <button
            onClick={nextSlide}
            className="absolute right-2 sm:right-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-20 hover:bg-opacity-30 text-white p-2 rounded-full"
          >
      ...
      {['Men', 'Women', 'Home Essentials'].map((mainCategory) => {
        ...
        return subCategorySamples.length === 0 ? null : (
          <section key={mainCategory} className="py-16 bg-white">
            <div className="container mx-auto px-4">
                    ...
                    <CardContent className="p-0">
                      <div className="relative overflow-hidden">
                        ...
                        <div className="absolute top-2 left-2">
                          <Badge className="bg-amber-600">{product.category}</Badge>
                        </div>
                      </div>
                      <div className="p-4 text-center">
                        <h3 className="text-lg sm:text-base font-semibold text-gray-800 mb-1">{product.name}</h3>
                        <p className="text-sm sm:text-xs text-gray-600 mb-3 line-clamp-2">
                          {product.description}
                        </p>
                        <div className="flex items-center justify-between">
                          <span className="text-lg sm:text-base font-bold text-amber-600">
                            Rs. {product.price.toLocaleString()}
                          </span>
                          <div className="flex space-x-2">
                            <Link to={`/product/${product.id}`}>
                              <Button variant="outline" size="sm" className="text-xs sm:text-[10px]">
                                View
                              </Button>
                            </Link>
                            {currentUser && !isAdmin && (
                              <Button
                                ...
                                size="sm"
                                className="bg-amber-600 hover:bg-amber-700 p-1"
                              >
              ...
              <div className="text-center mt-8">
                <Link to={`/products?mainCategory=${mainCategory}`}>
                  <Button size="lg" variant="outline" className="hover:bg-amber-100">
      ...
      <section className="py-16 bg-amber-50">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl sm:text-2xl font-bold mb-6 text-gray-800">Visit Our Store</h2>
              ...
              <div className="mt-6">
                <Link to="/contact">
                  <Button className="bg-amber-600 hover:bg-amber-700 text-sm sm:text-base">
                    Get Directions
                  </Button>
                </Link>
              </div>
            </div>
            <div className="bg-white p-6 rounded-lg shadow-lg mt-8 md:mt-0">
              <h3 className="text-xl sm:text-lg font-semibold mb-4 text-center">Why Choose Us?</h3>
